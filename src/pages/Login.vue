<template>
  <div>
    <q-form @submit="nextPressed" @reset="backPressed" class="q-gutter-md">
      <q-input
        filled
        v-model="number"
      ...
      />
      <q-input v-if="smsSent" v-model="code" filled label="4 Digit Code" />
      <div>
        <q-btn v-if="smsSent" label="Back" type="reset" color="primary" flat class="float-left" />
        <q-btn label="Next" type="submit" color="primary" class="float-right" />
      </div>
    </q-form>
  </div>
</template>
  ...
  methods: {
    ...
    submitCode() {
      ajax
        ...
        .catch(err => this.handleLoginFail(err.response));
    },
